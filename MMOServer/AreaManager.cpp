...
void PatrolErrorLinkWorkThread(LPVOID areasPtr, LPVOID hREv, LPVOID hBEv)
{
	...
	while (true)
	{
		if (ErrorHandlerPtr->IsGabStart == true)
		{
			printf("가비지 작동!\n");
			...
			ErrorHandlerPtr->IsGabStart = false;
...
CAreaManager::CAreaManager(int areaAmount)
{
	...
	hRemoveEvent = CreateEvent(NULL, TRUE, TRUE, NULL);	// 매뉴얼 리셋, 비신호 상태
	hBroadcastEvent = CreateEvent(NULL, FALSE, FALSE, NULL); // 매뉴얼 리셋, 신호 상태
